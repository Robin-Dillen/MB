...
Parser::varsToProds(const vector<Variable *> &variables) {
...
void Parser::moveLocSym(map < string, vector < vector < string
>>> &prods) {
for(
auto mapItem = prods.begin();
mapItem != prods.
...
swap(mapItem
->second[j][i+1], mapItem->second[j][i]);
}
}
}

void Parser::getClosure(const DFAState *rootState, map <string, vector<vector < string>>
...
-1 > j){
getClosure(rootState, mapItem
->second[i][j+1],closure);

}
}
}
for(
auto mapItem = closure.begin();
mapItem != closure.
...
prods[mapItem->first] = mapItem->
second;
}
else{
for(
auto production = mapItem->second.begin();
production != mapItem->second.

end();

production++){
if(!
count(prods[mapItem->first]
.

begin(), prods[mapItem->first]
...
void Parser::getClosure(const DFAState *rootstate, string item, map <string, vector<vector < string>>

> &closure) {
map <string, vector<vector < string>>>
root = rootstate->getContent();
if(root.
find(item)
== root.
...
Parser::goTo(const DFAState *rootState, string s) {
...
DFAState *Parser::stateExists(const map <string, vector<vector < string>>
...
void Parser::printState(map < string, vector < vector < string
>>> prods){
for(
auto it
: prods){
